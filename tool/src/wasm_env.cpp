...
std::map<String, StaticTask_t> taskBuffers;
...
m3ApiRawFunction(m3_arduino_pauseTask) {
    ...
    saveTaskState(filename, (uint8_t*)state, len);

    m3ApiSuccess();
}

m3ApiRawFunction(m3_arduino_saveTaskState) {
    m3ApiGetArgMem(const uint8_t*, state);
    m3ApiGetArg(uint32_t, len);
    m3ApiGetArgMem(const char*, filename);
...
void runWasmState(void* parameters) {
    const char* filename = static_cast<const char*>(parameters);
    IM3Runtime runtime = taskRuntimes[filename];

    callWasmFunction(runtime, "_state");
...
bool pauseTask(String filename) {
    if (taskHandles.count(filename) && eTaskGetState(taskHandles[filename]) != eSuspended) {
        vTaskSuspend(taskHandles[filename]);
        return true;
    }
    return false;
}
...
void removeTaskState(String filename) {
    if (!taskStates.count(filename)) {
        return;
    }
    TaskState taskState = taskStates[filename];
...
bool isTaskPaused(String filename) {
    if (taskHandles.count(filename)) {
        return eTaskGetState(taskHandles[filename]) == eSuspended;
    }
    return false;
}
